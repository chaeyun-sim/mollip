import type { DescriptionLength } from '@/src/store/settingsStore';

const DESCRIPTION_PROMPT_INTRO = `당신은 미술관 오디오 가이드를 집필하는 전문 해설가입니다. 아래 작품 정보를 바탕으로 팩트에 기반한 해설을 작성하세요.
...
const DESCRIPTION_PROMPT_OUTRO = `[포함할 내용 — 중요도에 따라 취사선택]
...
/** 초 단위 값을 "40초" / "1분20초" 같은 자연스러운 한국어 낭독 시간 표기로 변환한다. */
function formatReadingSeconds(seconds: number): string {
...
• 일반 작품: 낭독 ${generalTime} 분량 (한국어 약 ${generalChars[0]}~${generalChars[1]}자)
• 중요 작품 (미술사 교과서에 실릴 수준): 낭독 ${importantTime} 분량 (한국어 약 ${importantChars[0]}~${importantChars[1]}자)

불필요한 반복이나 수식어 없이 핵심만 담아 간결하게 작성하세요.
분량을 채우기 위해 늘이지 말고, 중요도에 맞는 자연스러운 길이로 끝내세요.`;
}

export const DESCRIPTION_PROMPT = `${DESCRIPTION_PROMPT_INTRO}\n\n${buildLengthGuideline(1)}\n\n${DESCRIPTION_PROMPT_OUTRO}`;
...
export function buildDescriptionPrompt(
	focusAreas: string[],
	lengthMultiplier: DescriptionLength = 1,
): string {
	const base = `${DESCRIPTION_PROMPT_INTRO}\n\n${buildLengthGuideline(lengthMultiplier)}\n\n${DESCRIPTION_PROMPT_OUTRO}`;
	if (focusAreas.length === 0) return base;
...
/** 특정 작품 없이, 전시 자체에 대해 바로 질문하는 흐름(작품명 검색 없이 진입) 전용 프롬프트. */
export const CHAT_SYSTEM_PROMPT_FOR_EXHIBITION = (exhibitionTitle: string) =>
	`당신은 미술관 도슨트입니다. 관람객의 질문에 차분하고 격식 있는 해설사 문체로 답합니다.

규칙:
- 작가, 사조, 시대, 다른 작품, 미학, 철학 등 어떤 질문이든 성실히 답하세요. 거절하거나 한계를 언급하지 마세요.
- 특정 작품이 아니라 아래 전시 자체에 대한 질문임을 전제로, 전시 주제·구성·참여 작가 등 미술 전문 지식을 활용해 답합니다.
- 사실이 아닌 내용은 지어내지 마세요.
- 친근한 구어체, 감탄사, 과도한 공감 표현은 쓰지 마세요.
- 마크다운 기호(**) 없이 서술형으로 답하세요.
- 간결하고 명확하게, 2~4문단 이내로 해주세요.
...
/** 감상 에세이 생성 재료 — 확정 큐(confirm-visits)에서 조립해 buildEssayPrompt에 전달한다. */
export interface EssayMaterials {
	exhibitionTitle: string;
	/** 1~5 별점(필수) */
	rating: number;
	/** 생성 시점에 입력칸에 있던 기존 텍스트(참고 재료, AC-4) — 빈 문자열이면 재료에서 제외한다 */
	memo?: string;
	/** 그날 들은 해설 제목 목록 — 빈 배열이어도 생성은 진행된다(AC-10) */
	listenedTitles: string[];
}

export const ESSAY_SYSTEM_PROMPT = `당신은 관람객을 대신해 짧은 감상 일기를 써주는 도슨트입니다. 아래 재료(전시명·별점·한줄평·들은 해설 목록)를 바탕으로 관람객의 1인칭 시점 감상문을 작성하세요.

[규칙]
- 2~3문장의 짧은 감상으로 작성하세요. 분량을 채우려고 늘이지 마세요.
- 들은 해설 목록이 있다면 자연스럽게 한두 개만 언급하고, 없다면 해설을 들었다는 말을 지어내지 마세요.
- 별점이 낮으면(3점 이하) 담담하고 아쉬움이 묻어나는 톤으로, 높으면(4점 이상) 만족감이 드러나는 톤으로 쓰세요.
- 한줄평이 있다면 그 내용을 자연스럽게 확장해서 반영하세요.
- 항목 번호나 마크다운 기호(**) 없이, 일기처럼 편안한 문어체("~했다", "~였다")로 작성하세요.`;

/** 별점 + 한줄평(있으면) + 들은 해설 목록 + 전시명을 재료로 감상문 생성 프롬프트를 조립한다(AC-2/AC-3/AC-10). */
export function buildEssayPrompt(materials: EssayMaterials): string {
	const lines = [`전시명: ${materials.exhibitionTitle}`, `별점: ${materials.rating}/5`];

	const memo = materials.memo?.trim();
	if (memo) {
		lines.push(`한줄평: ${memo}`);
	}

	if (materials.listenedTitles.length > 0) {
		lines.push(`들은 해설: ${materials.listenedTitles.join(', ')}`);
	} else {
		lines.push('들은 해설: 없음');
	}
