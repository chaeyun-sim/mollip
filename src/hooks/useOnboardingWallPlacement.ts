...
export function useOnboardingWallPlacement() {
 ...
 const genres = useMemo(() => normalizeOnboardingGenres(placements.flatMap((piece) => piece ? [piece.genre] : [])), [placements]);
 ...
 const initialize = useCallback((stored: string[]) => {
  const pieces = normalizeOnboardingGenres(stored).slice(0, 5).map((genre) => ONBOARDING_WALL_PIECES.find((piece) => piece.genre === genre && piece.representative) ?? null);
  setPlacements(Array.from({ length: 5 }, (_, index) => pieces[index] ?? null));
  setSelectedFrame(null);
  setAnnouncement('');
 }, []);
 const place = useCallback((piece: OnboardingWallPiece, target: number, source: number | null = null) => {
  ...
  setPlacements(next);
  setSelectedFrame(null);
  announce(`${piece.genre === '현대미술' ? '현대 미술' : piece.genre} 작품을 걸었어요. ${next.filter(Boolean).length}/5`);
 }, [placements, announce]);
 ...
 const cancel = useCallback(() => { setSelectedFrame(null); setAnnouncement(''); }, []);
 ...
 return { placements, selectedFrame, announcement, count, genres, initialize, place, selectFrame, cancel, remove, announce };
}
