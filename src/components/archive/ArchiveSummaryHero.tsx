...
function StatPill({ label, value }: { label: string; value: number }) {
	return (
		<View className="flex-row items-baseline gap-1.5 rounded-full border border-divider bg-white px-3.5 py-2">
			<Text className="text-[13px] font-pretendard-medium text-secondary">{label}</Text>
			<Text className="text-[15px] font-pretendard-semibold text-primary">{value}</Text>
		</View>
	);
}
...
export function ArchiveSummaryHero({ stats }: ArchiveSummaryHeroProps) {
	return (
		<View className="mb-5">
			<Text className="text-[26px] leading-[32px] font-hahmlet-bold text-primary">관람 기록</Text>
			<Text className="text-[14px] mt-2 leading-[21px] font-pretendard-regular text-secondary">
