...
function busColorFromNumber(routeName?: string): string {
  ...
  if (digits.length === 4) return digits.startsWith('9') ? BUS_ROUTE_TYPE_COLOR[14] : BUS_ROUTE_TYPE_COLOR[12];
...
const SUBWAY_NAME_COLOR: Array<{ match: string; color: string }> = [
...
function subwayColorFromName(routeName?: string): string {
  ...
  return SUBWAY_NAME_COLOR.find((line) => routeName.includes(line.match))?.color ?? LEG_DEFAULT_COLOR.subway;
}
