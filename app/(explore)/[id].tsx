...
import { ExhibitionTicketCTA } from '@/src/components/explore/ExhibitionImmersiveCTA';
			...
			>
				...
				<FadeInView delay={100}>
					...
					>
						...
						{splitArtistNames(exhibition.artist).map((name) => (
							<ExhibitionMetaPill key={name} icon='person-outline' text={name} />
						))}
						{exhibition.exhibitionType && (
							<ExhibitionMetaPill text={exhibition.exhibitionType} />
						)}
						{getGenreTag(exhibition) && (
							<ExhibitionMetaPill text={getGenreTag(exhibition)!} />
						)}
					</ScrollView>
				</FadeInView>
				...
				<FadeInView delay={300}>
					<View className='px-6 pt-10'>
						...
						<ExhibitionInfoRow label='관람료'>
							{exhibition.admission}
						</ExhibitionInfoRow>

						<ExhibitionInfoRow label='전시 형태' isLast>
