...
CLAUDE.md · `.claude/agents/` · `.claude/skills/` · `.claude/rules/` · 훅 변경만 기록한다(기능 코드 변경은 커밋 로그가 정본). 5행을 넘으면 가장 오래된 행부터 `.docs/HARNESS_CHANGELOG.md`로 이관한다(이관 시 내용 수정 금지, append-only). 전체 이력은 그 파일 참고.
